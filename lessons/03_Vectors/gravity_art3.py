...
class Game:
    ...
    def run(self):
        """Main game loop"""
        player = Player(self, 100, 300)
        player2 = Player(self, 200, 300)
        ...
        while self.running:
            ...
            player.update(pygame.K_s, pygame.K_d, pygame.K_a, pygame.K_w)
            player2.update(pygame.K_DOWN, pygame.K_RIGHT, pygame.K_LEFT, pygame.K_UP)

            #self.screen.fill(Colors.BACKGROUND_COLOR)
            player.draw(self.screen, (self.myVar%2 or 255, math.fabs(self.myVar%510 - 255), 100))
            player2.draw(self.screen, (self.myVar%255, self.myVar%100, 200))
            self.myVar += 1
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, game: Game, player_start_x, player_start_y):
        super().__init__()
        self.game = game
        ...
        # Player position
        self.pos = pygame.Vector2(player_start_x, 
                                  player_start_y if player_start_y is not None else settings.height - self.height)
    ...
    def draw(self, screen, color):
        ...
        initial_position = self.pos
        end_position = self.pos + self.v_jump * self.LENGTH * 100
        pygame.draw.line(screen, color, initial_position, end_position, 2)
        self.myVar += 1
